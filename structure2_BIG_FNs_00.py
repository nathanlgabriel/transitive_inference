...
def single_play(fnoise2, fplen2, falen2, fmaxval2, fsides2, frein2, fpunish2, frecweights2, fsigweights2, fstate2, frandunif22, frandunif42, frandunif62, frandunif82, fpairs2):
    ...
    for idx123 in range(0, fmaxval2):
        xl = leftweights[0]
        xr = rightweights[0]
        if xl < leftsumrand[idx123]:
            leftval +=1
        if xr < rightsumrand[idx123]:
            rightval +=1
    ...
    if leftstate > rightstate:
        ...
        if recpick == 0:
            ...
            leftweights[0] += frein2*(fmaxval2-leftval)
            leftweights[1] += frein2*leftval
            rightweights[0] += frein2*(fmaxval2-rightval)
            rightweights[1] += frein2*rightval
            
            recweights[0] += frein2
        else:
            
            leftweights[0] += fpunish2*(fmaxval2-leftval)
            leftweights[1] += fpunish2*leftval
            rightweights[0] += fpunish2*(fmaxval2-rightval)
            rightweights[1] += fpunish2*rightval
...
def play_sequence(n, rng, rein1, punish1, rein2, punish2, timesteps, nsteps, sides, pairs, testpairs, nonadjpairs, allpairs, plen, alen, terms, maxvalue, startstop, noise, annealing, runs, inertia, blocklength):
    ...
    sigweights = inertia*np.ones([sides, 2, terms, 2])
